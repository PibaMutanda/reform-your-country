...
public class BlackBeltTagParser {
	...
	protected boolean errorFormat = false;
	...
	public BlackBeltTagParser(BlackBeltTagHandler handler, String inputString){
		this.input = inputString == null ? "" : inputString;
		...
		if(handler != null){
			...
			throw new RuntimeException("The BlackBeltTagHandler can not be null !");
		}
	}
	...
	private boolean isInSupportedTagNames(String searchedTag){
	    return Arrays.asList(SUPPORTED_TAG_NAMES).contains(searchedTag);
    }
	...
	protected void onElement(Element element) {
		...
		if ("image".equals(element.name)) {
			...
			blackBeltTagHandler.onUnknownElement(element);
