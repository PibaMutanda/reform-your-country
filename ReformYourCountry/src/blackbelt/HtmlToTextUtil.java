...
/**
 * Class to convert arbitrary HTML documents into text. Conversion is very basic, intended
 * to be used to strip markup from HTML-only emails for inclusion.
 */
public class HtmlToTextUtil {
	private static final Logger log = Logger.getLogger(HtmlToTextUtil.class);

	private static class HTMLCallbackHandler extends
		...
		public void handleStartTag(HTML.Tag tag,
				javax.swing.text.MutableAttributeSet set, int position) {
			try {
				if (inBody && started && tag.equals(HTML.Tag.P)) {
					out.write(NEWLINE + NEWLINE);
				} else if (inBody && started && tag.equals(HTML.Tag.OL)
						|| tag.equals(HTML.Tag.UL)) {
					inList = true;
					out.write(NEWLINE + NEWLINE);
					if (tag.equals(HTML.Tag.OL))
						listCount = 1;
				} else if (inBody && started && inList
						&& tag.equals(HTML.Tag.LI)) {
					out.write(NEWLINE);
					if (listCount != -1) {
					...
					out.write(NEWLINE);
					firstTD = true;
				} else if (inBody && started && tag.equals(HTML.Tag.TD)
						|| tag.equals(HTML.Tag.TH)) {
					if (!firstTD) {
						out.write(TAB);
					} else {
						firstTD = false;
				...
				} else if (inBody && started && tag.equals(HTML.Tag.H1)
						|| tag.equals(HTML.Tag.H2) || tag.equals(HTML.Tag.H3)
						|| tag.equals(HTML.Tag.H4) || tag.equals(HTML.Tag.H5)
		...
		public void handleEndTag(HTML.Tag tag, int position) {
			if (inBody && started && tag.equals(HTML.Tag.OL)
					|| tag.equals(HTML.Tag.UL)) {
				inList = false;
				if (tag.equals(HTML.Tag.OL))
					listCount = -1;
				...
				if (links.size() != 0) {
					...
					try {
						...
						for (int i = 0; i < links.size(); i++) {
							...
							out.write(OPEN_BRACKET + (i + 1) + CLOSE_BRACKET
									+ SPACE + src);
							if ((i + 1) < links.size()) {
								out.write(NEWLINE);
							}
		...
		public void handleSimpleTag(HTML.Tag tag,
				javax.swing.text.MutableAttributeSet a, int pos) {
			try {
	...
	public static String convert(String html) {
		...
		try {
			...
			// can't occur with StringWriter
			log.error(e);
		}
		return out.toString();
	}

	private static void convert(Reader reader, Writer writer)
			throws IOException {
